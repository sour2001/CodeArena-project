...
const showData = (articles) => {
    ...
    if (artitem.length > 0) {
        ...
        for (var i = 0; i < artitem.length; i++) {
            ...
            var output = "";
            output += `
    <div class="card text-center" style="border-color:#f4511e;background-color:transparent;margin-bottom:10px">
     <div class="card-body ">
     <p class="card-text font-weight-bold btn-color card-header ">${artitem[i].name}</p>
     <p class="card-text card-header" style="color:white">Duration : ${artitem[i].duration}</p>
     <p class="card-text card-header" style="color:white"> ${artitem[i].toStart}</p>
