...
const showData = (articles) => {
    ...
    <div class="card text-center border-color" style="border-color:#f4511e;background:transparent" >
     <div class="card-body" style="background-color:transparent;">
     <p class="card-text btn-color font-weight-bold card-header" style="background-color:transparent;">${artitem[i].name}</p>
     <p class="card-text card-header"style="background-color:transparent;color:white">${artitem[i].day} ${artitem[i].month} ${artitem[i].time}  <sup>IST</sup></p>
    <a href="${artitem[i].link}" class="btn btn-color" style="border-color:#f4511e;">CONTEST PAGE</a>
       </div>
     </div>`
